...
class CallBackVerification(object):
    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        ...
        for i in range(len(self.ver_list)):
            acc1, std1, acc2, std2, val, val_std, xnorm, embeddings_list = verification.test(
                self.ver_list[i], backbone, 10, 10)
            logging.info('[%s][%d]XNorm: %f' % (self.ver_name_list[i], global_step, xnorm))
            logging.info('[%s][%d]Accuracy-Flip: %1.5f+-%1.5f' % (self.ver_name_list[i], global_step, acc2, std2))
            logging.info('[%s][%d]Validation: %1.5f+-%1.5f' % (self.ver_name_list[i], global_step, val, val_std))

            self.summary_writer: SummaryWriter
            self.summary_writer.add_scalar(tag=self.ver_name_list[i], scalar_value=acc2, global_step=global_step, )
            self.summary_writer.add_scalar(tag='XNorm_' + self.ver_name_list[i], scalar_value=xnorm,
                                           global_step=global_step, )
            self.summary_writer.add_scalar(tag=self.ver_name_list[i] + '_val', scalar_value=val,
                                           global_step=global_step, )
        ...
        if self.rank == 0 and global_step > 0 and global_step % self.frequent == 0:
            if self.init:
                ...
                if self.writer is not None:
                    ...
                    self.writer.add_scalar('acc', acc, global_step)
                if fp16:
                    msg = "Speed %.2f samples/sec   Loss %.4f   Acc %.2f   LearningRate %.6f   Epoch: %d   Global Step: %d   " \
                          "Fp16 Grad Scale: %2.f   Required: %1.f hours" % (
                              speed_total, loss.avg, acc, learning_rate, epoch, global_step,
